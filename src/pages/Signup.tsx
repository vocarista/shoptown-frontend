...
const Signup = () => {
    ...
    async function signupHandler() {
        ...
        if (username === '' || password === '' || firstName === '' || lastName === '') {
            setErrorDescription('Please fill all fields');
            setShowError(true);
            return;
        }

        axios.post(`${base}/user/auth/is-username-available`, {
            username: username
        }).then((response: AxiosResponse) => {
            if (response.status === 200) {
                setIsUsernameAvailable(response.data);
            } else {
                setErrorDescription('Unable to check username availability');
                setShowError(true);
            }
        ...
        if (isUsernameAvailable) {
            axios.post(`${base}/user/auth/register`, {
                firstName: firstName,
                lastName: lastName,
                username: username,
                password: password
            ...
            .then((response: AxiosResponse) => {
                if (response.status === 200) {
                    ...
                    setErrorDescription('Unable to register user');
                    setShowError(true);
                }
            })
            .catch((error: AxiosError) => {
            ...
            setErrorDescription('Username is not available');
            setShowError(true);
        }
    }
